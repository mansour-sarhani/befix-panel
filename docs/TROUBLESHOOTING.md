...
**✅ CORRECT (Fixed):**
```javascript
const token = await getAuthToken();  // Reads 'befix_auth_token' cookie
```

### Why This Happened
1. Cookie is named `befix_auth_token` (defined in `src/constants/config.js`)
...
3. Should see `befix_auth_token` cookie (httpOnly)
...
FIREBASE_CLIENT_EMAIL=firebase-adminsdk-xxx@befix-panel.iam.gserviceaccount.com
...
1. Go to [Firebase Console](https://console.firebase.google.com/project/befix-panel/settings/serviceaccounts/adminsdk)
...
console.log(document.cookie);  // Won't show befix_auth_token
